...
class Webservices1 extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model(array('Ws_model', 'Outlet_model'));
        ...
        if (isset($_POST['visit'])) {
            ...
            if ($visit_id) {
                ...
                while (isset($_FILES['before' . $j])) {
                    ...
                    $before_img = date('Y') . '-before' . time() . '.jpg';
                    $after_img = date('Y') . '-after' . time() . '.jpg';
...
//save pictures into database
    ...
    function saveVisitGroupWithMail() {
        if (isset($_POST['visitGroup'])) {
            ...
            $final = date("Y", strtotime("+0 month", $date1));
            $visit['year'] = $final;

            $final2 = date("m", strtotime("+0 month", $date1));
            $quarter = round($final2 / 3);
            if ($quarter == 0) {
                $quarter = 1;
            }
    ...
    function channels() {
        $ha = $this->Ws_model->get_ws_channel();
        print(json_encode($ha));
    }
    ...
    function getHaByAdmin($admin_id) {
        $ha_products = $this->Ws_model->get_ws_ha_products($admin_id);
    ...
    //correct visit_unique_id_model
    function correctDB() {
        $visit_array = $this->Ws_model->get_ws_visits();
        print(json_encode($visit_array));
        die("amira");
        foreach ($visit_array as $visit) {
            if (isset($visit->uniqueId)) {
                $visitUniqueId = $visit->uniqueId;
                $model_array = $this->Ws_model->get_ws_visits($visit->id);

                foreach ($model_array as $model) {
                    $this->Ws_model->update_model_uniqueID($visit_id, $visitUniqueId);
                }
            }
        }
    }
