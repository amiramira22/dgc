...
Class Dashboard_model extends CI_Model {
    ...
    function get_target_visit_by_admin($admin_id, $today) {
        ...
        $this->db->select('
		count(bcc_outlets.id) as nb_visits,
		', false);
   ...
   function get_daily_visit_by_admin($admin_id,$start_date,$end_date) {
		...
		$this->db->select('
		count(det_visits.id) as nb_visits,
		',false);
    ...
    function get_brand_multiple_date_data_for_shelf($category_id) {
        ...
        $this->db->select('sum(bcc_models.av) as oos_perc, visits.w_date as date,
		sum(bcc_models.av_sku) as sum_av_sku,count(bcc_models.id) as total,sum(bcc_models.shelf) as shelf,
		,brands.name as brand_name,brands.color as brand_color,zones.name as zone_name,outlets.channel as channel_name,
		zones.color as zone_color,channels.color as channel_color,
		count(bcc_models.id) as total,
		zones.name as zone ,
		count(CASE WHEN bcc_models.av = 1 THEN bcc_models.av ELSE 0 END)  AS count_av,
		sum(bcc_models.av_sku) as sum_av_sku,sum(bcc_models.av) as oos_perc
        ...
        foreach ($query as $row) {
            ...
            if ($row['total'] != 0) {
                    ...
                    $av = number_format(($row['oos_perc'] / $row['total']) * 100, 2, '.', ' ');
                    $oos = 100 - $av;
    ...
    function get_monthly_remarks_visits() {
		...
		visits.date as date,visits.active as active,visits.modified as modified,outlets.super_market_project as super_market_project,admin.photos as photos,
		remark as remark,visits.av_perc as av_perc,visits.oos_perc as oos_perc,visits.entry_time as entry_time,visits.exit_time as exit_time,visits.was_there as was_there');
    ...
    function get_brand_json_data($brand_id, $m_date) {
        $this->db->select('
		count(bcc_models.id) as total,
		zones.name as zone ,
        ...
        foreach ($query as $row) {
            ...
            if ($row['total'] != 0) {
                $row_data['zone'] = $row['channel'];
                if ($brand_id == 18) {
                    $row_data['av'] = number_format(($row['count_av'] / $row['total']) * 100, 2, '.', ' ');
    ...
    function get_by_brand_json_data($brand_id, $m_date) {
        $this->db->select('
		count(bcc_models.id) as total,
		zones.name as zone ,
		brands.name as brand_name,
        ...
        foreach ($query as $row) {
            ...
            if ($row['total'] != 0) {
                ...
                if ($brand_id == 18) {
                    ...
                    $row_data2['value'] = number_format(($row['count_av'] / $row['total']) * 100, 2, '.', ' ');
                    $row_data3['title'] = "OOS";
                    $row_data3['color'] = "#FF0000";
                    $row_data3['value'] = 100 - $row_data['av'];
                } else {
                    ...
                    $row_data2['title'] = "AV";
                    $row_data2['color'] = "#08AF02";
                    $row_data2['value'] = number_format(($row['sum_av_sku'] / $row['total']) * 100, 2, '.', ' ');
                    $row_data3['title'] = "OOS";
                    $row_data3['color'] = "#FF0000";
                    $row_data3['value'] = 100 - $row_data2['value'];
                }
    ...
    function get_brand_json_data_super($brand_id, $m_date) {
        $this->db->select('
		count(bcc_models.id) as total,
    ...
    function get_distinct_categories($brand_id, $m_date) {

        $this->db->select('categories.name as category_name,categories.color as color', false);
        $this->db->from('visits');
    ...
    function rr($brand_id, $m_date) {

        $this->db->select('outlets.zone as zone,models.brand_id as brand_id,'
                . 'sum(bcc_models.av) as oos_perc,'
                . 'sum(bcc_models.av_sku) as sum_av_sku,'
                . 'count(bcc_models.id) as total,'
                . 'brands.name as brand_name,'
                . 'categories.name as category_name,'
                . 'categories.color as color', false);
        $this->db->from('visits');
